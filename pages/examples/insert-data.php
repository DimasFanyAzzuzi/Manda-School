<?php
   ...
   // Input File
   $ChooseFoto = strtolower($_FILES['image_guru']['name']); // Mengambil nama file
   $Patch = $_FILES['image_guru']['tmp_name']; // Mengambil lokasi file
   $CheckInput = $_FILES['image_guru']['error']; // Mengambil bentuk error ketika upload file
   $CheckSize = $_FILES['image_guru']['size']; // Mengambil ukuran file yang di-upload
   
   // Menampung lokasi pemindahan file ke dalam variabel $MoveTO
   $MoveTO = "../../assets/img/profil/";
   ...
   // }
      ...
      mysqli_query($koneksi, "INSERT INTO tbl_guru (image_guru, nama_guru, jenis_kelamin, no_hp, tgl_lahir, nip, password_guru, alamat, id_mapel, id_admin) VALUES ('$SetUniqName', '$InputNama', '$ChooseKL', '$InputHp', '$InputTgl', '$InputNip', '$InputPs', '$InputAla', '$InputMpl', '$InputAdm')");
      echo "
				<script>
					setTimeout(function() { 
						Swal.fire({
